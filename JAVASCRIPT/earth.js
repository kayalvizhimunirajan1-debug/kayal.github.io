(function() {
  ...
  // Check if Earth PNG exists
  if (earthImage) {
    earthImage.onerror = function() {
      console.warn('Earth PNG not found! Please place earth.png in the same folder.');
      ...
      if (wrapper) {
        ...
        errorMsg.innerHTML = '⚠️ earth.png not found<br>Add your Earth image to this folder';
    ...
    earthImage.onload = function() {
      console.log('Earth PNG loaded successfully');
    };
  }
  
  function rotateEarth(timestamp) {
    if (!lastTimestamp) {
      lastTimestamp = timestamp;
      animationId = requestAnimationFrame(rotateEarth);
      return;
    }
    ...
    if (isRotating && earthImage) {
      // Update rotation based on speed (degrees per second)
      currentRotation += rotationSpeed * deltaTime;
      currentRotation = currentRotation % 360;
      earthImage.style.transform = `rotate(${currentRotation}deg)`;
    }
  ...
  if (resetBtn) {
    resetBtn.addEventListener('click', () => {
      currentRotation = 0;
      if (earthImage) {
        earthImage.style.transform = `rotate(0deg)`;
      }
    });
  }
  
  // Hover effect for faster rotation (optional)
  const earthWrapper = document.querySelector('.earth-rotation-wrapper');
  ...
  if (orbitCanvas) {
    ...
    function drawOrbitSimulation(angleDeg) {
      ...
      orbitCtx.fillText(" Sun", centerX-15, centerY-32);
